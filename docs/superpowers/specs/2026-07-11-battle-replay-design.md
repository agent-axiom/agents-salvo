...
Playback uses a small replay clock that owns one interval handle plus UI state fields for playing status and speed index. Starting playback clears any stale interval before creating a new one. Each tick advances one move and updates only the replay subtree, preserving the result modal scroll position and the focused replay control. A stable live region outside the replaced subtree announces the selected move. Manual Previous/Next navigation stops playback first. Reaching the final move stops the interval and updates the Play/Pause label. Changing speed while playing restarts the single interval at the selected delay without changing the current move.

## Testing

Add behavioral unit tests for replay turn transitions, speed cycling, and single-interval ownership. Keep structural UX tests for playback state, focus and scroll preservation, renderer, map builder, active-cell pulse, controls, action handlers, CSS hooks, and i18n keys. Extend the i18n coverage test to require playback labels and announcements in English, Russian, and Chinese. Run the full test, 98% coverage, build, and GitHub Pages deployment checks.
